...
#define LABEL_DRONE_TXT_PATH "./model/drone_labels_list.txt"
...
{
    printf("load lable %s\n", locationFilename);
    label_count = readLines(locationFilename, label, OBJ_CLASS_NUM_MAX);
    return 0;
}

static int get_tensor_elem_num(const rknn_tensor_attr* attr)
{
    int n = 1;
...
static int process_u8(uint8_t *box_tensor, int32_t box_zp, float box_scale,
                      ...
                      int grid_h, int grid_w, int stride, int dfl_len, int cls_num,
...
int post_process(rknn_app_context_t *app_ctx, void *outputs, letterbox_t *letter_box, float conf_threshold, float nms_threshold, object_detect_result_list *od_results)
{
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        if (cls_num <= 0)
        {
            int score_elem = get_tensor_elem_num(&app_ctx->output_attrs[score_idx]);
            cls_num = score_elem / (grid_h * grid_w);
        }
        ...
        if (app_ctx->is_quant) {
            validCount += process_i8_rv1106((int8_t *)_outputs[box_idx]->virt_addr, app_ctx->output_attrs[box_idx].zp, app_ctx->output_attrs[box_idx].scale,
                                ...
                                grid_h, grid_w, stride, dfl_len, cls_num, filterBoxes, objProbs, classId, conf_threshold);
        }
        else
        {
        ...
        if (app_ctx->is_quant)
        {
            ...
            validCount += process_u8((uint8_t *)_outputs[box_idx].buf, app_ctx->output_attrs[box_idx].zp, app_ctx->output_attrs[box_idx].scale,
                                     ...
                                     grid_h, grid_w, stride, dfl_len, cls_num,
...
{
    int ret = 0;
    FILE *fp = fopen(LABEL_DRONE_TXT_PATH, "r");
    if (fp)
    {
        fclose(fp);
        ret = loadLabelName(LABEL_DRONE_TXT_PATH, labels);
    }
    else
    {
